...
class Game
  ...
  def initialize(player_1 = ComputerPlayer.new("X"), player_2 = ComputerPlayer.new("O"))
    @board = Board.new
    if player_1.class == ComputerPlayer
      @player_1 = player_1
    else
      @player_1 = HumanPlayer.new(player_1, "X")
    end

    if player_2.class == ComputerPlayer
      @player_2 = player_2
    else
      @player_2 = HumanPlayer.new(player_2, "O")
    end
...
class HumanPlayer < Game
  ...
  def desired_position
    puts "Where would you like to put an #{mark}?"
    position = gets.chomp
...
if number == 0
  ...
  puts "Enter the name of your first player"
  player1 = gets.chomp
  puts "Enter the name of your second player"
  player2 = gets.chomp
  b = Game.new(player1, player2)
end
